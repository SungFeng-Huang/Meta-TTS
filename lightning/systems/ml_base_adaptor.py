...
class BaseAdaptorSystem(System):
    ...
    def forward_learner(
        ...
        mel_masks = (
            get_mask_from_lengths(
                mel_lens, max_mel_len) if mel_lens is not None else None
        )
    ...
    def meta_learn(self, batch, batch_idx, train=True):
        learner = self.adapt(batch, min(
            self.adaptation_steps, self.test_adaptation_steps), train=train)
