...
def test_adam():
    ...
    adam_0 = model_0.configure_optimizers()
    adam_1 = AdamWMAML(model_1, lr=0.1, first_order=True, weight_decay=0.1)

    train_data = DataLoader(RandomDataset(32, 64), batch_size=2)

    batch = list(train_data)[0]
    output_0 = model_0.training_step(batch, 0)
    output_1 = model_1.training_step(batch, 0)
    ...
    for n in state_dict_0:
        assert_close(state_dict_1[n], state_dict_0[n])
